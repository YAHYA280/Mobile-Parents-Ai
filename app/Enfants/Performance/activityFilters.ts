...
export const useActivityFilters = (activities: Activity[]) => {
  ...
  useEffect(() => {
    ...
    if (filters.advancedFilters.scoreRange.min > 0 || filters.advancedFilters.scoreRange.max < 100) {
      result = result.filter(activity => {
        if (!activity.score || !activity.score.includes('/')) return true;
        
        const [score, total] = activity.score.split('/').map(num => parseInt(num, 10));
        const percentage = (score / total) * 100;
        
        return percentage >= filters.advancedFilters.scoreRange.min && 
