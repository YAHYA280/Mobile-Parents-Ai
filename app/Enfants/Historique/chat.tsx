...
import { 
  View, Text, Alert, Platform, TextInput, 
  ScrollView, SafeAreaView, TouchableOpacity, ActivityIndicator, KeyboardAvoidingView
} from 'react-native';
...
const ASSISTANT_THEME: { [key: string]: { colors: [string, string], icon: string } } = {
  "J'Apprends": {
...
const ChatScreen = () => {
  ...
  const handleBack = () => {
    if (fromDetails) {
      ...
      router.push(`/Enfants/Historique/historydetails?activityId=${activityId}&childId=${childId}`);
    }
  };
  ...
  if (!activity || !child) {
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: colors.background }}>
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          <FontAwesome5 name="exclamation-circle" size={64} color={dark ? COLORS.white : COLORS.black} />
          <Text style={{ marginTop: 20, color: dark ? COLORS.white : COLORS.black }}>
          ...
          <View style={{
            ...
            <View style={{
              ...
              <TextInput
                ...
                editable={false}
              />
              <Ionicons name="lock-closed" size={18} color={dark ? COLORS.secondaryWhite : COLORS.gray3} />
            </View>
          </View>
          
          {/* Info Banner */}
          <View style={{
            backgroundColor: dark ? 'rgba(0, 149, 255, 0.1)' : 'rgba(0, 149, 255, 0.05)',
            padding: 12,
            alignItems: 'center',
            flexDirection: 'row',
            justifyContent: 'center'
